...
subjlist_infants = ['sub-CC00306XX09']
...
if reload_data:
    ...
    for hemiind, hemi in enumerate(['R', 'L']):
        ...
        if infants == 1:
            ...
            for subind, sub in enumerate(subjlist_infants):
                ...
                remotepath_conn = (
                    f'Results/{sub}_infants_tractography_results_VOXEL_R.npy')
                s3.download_file('smartontheinside', remotepath_conn,
                                os.path.join(analysis_root, f'{sub}_tractography_results_VOXEL_{hemi}.npy'))
                tract = np.load(
                    os.path.join(f'/home/chiaracaldinelli/rerun_roi97/{sub}_infants_tractography_results_VOXEL_{hemi}.npy'), allow_pickle=True)
                # tract = np.load(
    ...
    if infants == 1:
        np.save(os.path.join(analysis_root, f'results/conn_for_classifier_N-{nsub}_infants.npy'), conn_for_classifier)
        s3.upload_file(os.path.join(analysis_root, f'results/conn_for_classifier_N-{nsub}_infants.npy'),
                'smartontheinside', f'infant_classifier/conn_for_classifier_N-{nsub}_infants.npy')
    else:
