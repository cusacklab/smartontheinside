...
### POOL SUBJECTS
subjlist = ['106521'] # cancella questo perche e doppio
...
for hemiind, hemi in enumerate(['R','L']):
    ...
    for sub in range(nsub):
        print(f'Working on subject {subjlist[sub]} connectivity data, {hemi} hemisphere')
        
        remotepath = (f'HCP_1200/{subjlist[sub]}/T1w/Diffusion.probtrackx2/{subjlist[sub]}_tractography_results_VOXEL_R.npy')
        s3.download_file('smartontheinside', remotepath, f'/Users/chiara/{subjlist[sub]}_tractography_results_VOXEL_R.npy')
        tract = np.load(f'/Users/chiara/{subjlist[sub]}_tractography_results_VOXEL_R.npy', allow_pickle=True)

        aj = np.load(f'/Users/chiara/{subjlist[sub]}_tractography_results_VOXEL_R.npy', allow_pickle=True)
        for target_roi in range(1, 361): # For every target ROI (334 in total)
            conn.extend(tract[target_roi, :])
        print(len(conn))

    np.save(f'/Users/chiara/conn_for_classifier_{hemi}.npy', conn)
...
# evaluate model
scores = cross_val_score(model, act_for_classifier, conn, scoring='neg_mean_absolute_error', cv=cv, n_jobs=-1)
