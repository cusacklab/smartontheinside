...
for hem in L R ; do
    ...
    aws s3 sync s3://smartontheinside/infant_tractography/${SUBJ}/Diffusion.probtrackx2/${hem}/ ${tmp_dir}/$SUBJ/Diffusion.probtrackx2/${hem}/
    ...
    for roi in "${roilist[@]}"; do
        ...
        if [[  " ${DLPFroilist[*]} " =~ " ${roi} " ]]; then
            ...
            applywarp -i ${tmp_dir}/$SUBJ/Diffusion.probtrackx2/${hem}/seeds_to_glasser_labels_dhcp_40weeks_LR_${SUBJ}_${roi}.nii.gz \
                        -o ${tmp_dir}/$SUBJ/Diffusion.probtrackx2/${hem}/seed2target_to_infant_template_${SUBJ}_${roi}.nii.gz \
                        -w /dhcp/dhcp_dmri_pipeline/${SUBJ}/${SESS}/xfm/${SUBJ}_${SESS}_from-dwi_to-template40wk_mode-image.nii.gz \
                        -r /dhcp/rhodri_registration/atlases/dhcp_volume_40weeks/template_t1_1mm.nii.gz     
            ...
            antsApplyTransforms -i ${tmp_dir}/$SUBJ/Diffusion.probtrackx2/${hem}/seed2target_to_infant_template_${SUBJ}_${roi}.nii.gz \
                ...
                -o ${tmp_dir}/$SUBJ/Diffusion.probtrackx2/${hem}/seed2target_to_MNI_template_${SUBJ}_${roi}.nii.gz
            ...
            wb_command -volume-to-surface-mapping ${tmp_dir}/$SUBJ/Diffusion.probtrackx2/${hem}/seed2target_to_MNI_template_${SUBJ}_${roi}.nii.gz \
                ...
                -enclosing \
                ${tmp_dir}/$SUBJ/Diffusion.probtrackx2/${hem}/seeds_to_ROI_MNI_template.${roi}.shape.gii
