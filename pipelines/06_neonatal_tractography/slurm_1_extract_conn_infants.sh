            ...
            for SESSDIR in ${SUBJDIR}/ses-*/; do
                if [[ -d "$SESSDIR" ]]; then
                    ...
                    if [[ -f "${DWIPTH}/${SUBJ}/${SESS}/dwi.bedpostX/merged_f1samples.nii.gz" ]]; then
                        echo "running infant transformations for $SUBJ "
                        export SUBJ=$SUBJ 
                        sbatch --export=ALL,sub=$sub /home/chiaracaldinelli/smartontheinside/smartontheinside/slurm_2_extract_conn_infants.sh	
                        fi
		        fi 	  
            done
        fi
