...
nsub = 155
folder = f'/home/chiaracaldinelli/'
...
for hemiind, hemi in enumerate(['R', 'L']):
    ...
    df = pd.read_csv(os.path.join(folder, f'summary_N-{nsub}_revisions.csv'), index_col=False)
    print(df)
    ...
    if infants == 1:
        ...
        plt.savefig(os.path.join(folder, f'summarise_res_classifier_infants_pearson_{hemi}_for_revisions.png'), bbox_inches='tight')
        print(f'Figure saved as summarise_res_infants_pearson_{hemi}_for_revisions.png')
    else:
        plt.savefig(os.path.join(folder, f'summarise_res_classifier_pearson_{hemi}_for_revisions.png'), bbox_inches='tight')
        print(f'Figure saved as summarise_res_pearson_{hemi}_for_revisions.png')
...
for hemiind, hemi in enumerate(['R', 'L']):
    ...
    # Load values
    df = pd.read_csv(os.path.join(folder, f'summary_N-{nsub}_revisions.csv'))
...
#            res = scipy.stats.bootstrap((diff,), np.mean)
    ...
    plt.savefig((f'/home/chiaracaldinelli/smartontheinside/smartontheinside/matrix_{hemi}_hemisphere_for_revisions.png'), bbox_inches='tight')
