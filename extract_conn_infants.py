...
for sub in subjlist:
    ...
    # file_key = f'Results/{sub}_infants_tractography_results_VOXEL_L.npy'
    file_key = f'Results/{sub}_100_infants_tractography_results_VOXEL_L.npy'
    ...
    if 'Contents' in result:
        ...
        for hemiind, hemi in enumerate(['R','L']):
            ...
            for target_roi in range(1, 361): # For every target ROI (334 in total)
                if not target_roi in frontalregs_right:
                    if not target_roi in frontalregs_left:
                        ...
                        # s3.download_file(bucket, remotepath, f'/home/chiaracaldinelli/{sub}_infants_seeds_to_ROI.{target_roi}.shape.gii') 
                        # img_s2t = nib.load(f'/home/chiaracaldinelli/{sub}_infants_seeds_to_ROI.{target_roi}.shape.gii') 
                        img_s2t = nib.load(f'/home/chiaracaldinelli/rerun_roi97/T1w/Diffusion.probtrackx2/{hemi}/seeds_to_ROI_MNI_template.{target_roi}.shape.gii')
            ...
            # np.save((f'/home/chiaracaldinelli/{sub}_infants_tractography_results_VOXEL_{hemi}.npy'), vox_res)

            np.save((f'/home/chiaracaldinelli/rerun_roi97/{sub}_infants_tractography_results_VOXEL_{hemi}.npy'), vox_res)

        # s3.upload_file(f'/home/chiaracaldinelli/{sub}_infants_tractography_results_VOXEL_L.npy', 'smartontheinside', f'Results/{sub}_infants_tractography_results_VOXEL_L.npy')
        # s3.upload_file(f'/home/chiaracaldinelli/{sub}_infants_tractography_results_VOXEL_R.npy', 'smartontheinside', f'Results/{sub}_infants_tractography_results_VOXEL_R.npy')
        
        # os.remove(f'/home/chiaracaldinelli/{sub}_infants_tractography_results_VOXEL_L.npy')
        # os.remove(f'/home/chiaracaldinelli/{sub}_infants_tractography_results_VOXEL_R.npy')