            ...
            for target_roi in range(1, 361): # For every target ROI (334 in total)
                if not target_roi in frontalregs_right:
                    if not target_roi in frontalregs_left:
                        # remotepath = 'infant_tractography/sub-CC00907XX16/T1w/Diffusion.probtrackx2/R/seeds_to_ROI_MNI_template.1.shape.gii'
                        remotepath = f'infant_tractography/{sub}/T1w/Diffusion.probtrackx2/{hemi}/seeds_to_ROI_MNI_template.{target_roi}.shape.gii'
                        # Credentials for uploading data to the cusack lab s3
                        session = boto3.Session(profile_name='default')
                        s3 = session.client('s3')
                        bucket = 'smartontheinside'
                        ...
                        s3.download_file(bucket, remotepath, f'/home/chiaracaldinelli/{sub}_seeds_to_ROI_MNI_template.{target_roi}.shape.gii')
                        img_s2t = nib.load(f'/home/chiaracaldinelli/{sub}_seeds_to_ROI_MNI_template.{target_roi}.shape.gii')
