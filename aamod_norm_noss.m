...
function [aap,resp]=aamod_norm_noss(aap,task,varargin)
resp='';

% Allow flexiblity of domain and number of indices
...
switch task
    ...
    case 'doit'
        ...
        %% Get structural or other file to run normalization on
        inStream = aap.tasklist.currenttask.inputstreams(1).stream{1};

        domain=aap.tasklist.currenttask.domain;
        
        Simg = aas_getfiles_bystream(aap,domain,indices,inStream);
...
%             estopts.regtype='mni';    % turn on affine again
            ...
            aap=aas_desc_outputs(aap,domain,indices,'normalisation_seg_sn',SNmat);
            aap=aas_desc_outputs(aap,domain,indices,'normalisation_seg_inv_sn',invSNmat);
            ...
            for tissind=1:3
                aap=aas_desc_outputs(aap,domain,indices,sprintf('tissue_%s',tiss{tissind}),fullfile(Spth,sprintf('wc%d%s',tissind,[mSnme mSext])));
                aap=aas_desc_outputs(aap,domain,indices,sprintf('unmod_tissue_%s',tiss{tissind}),fullfile(Spth,sprintf('c%d%s',tissind,[mSnme mSext])));
            end
...
% SPM, AA
Simg = aas_getfiles_bystream(aap,domain,indices,'structural','output'); 
Simg = Simg(1,:);
