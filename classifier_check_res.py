...
for hemiind, hemi in enumerate(['R', 'L']):
    for taskind, task in enumerate(tasks_selected):
        ...
        ax[taskind][hemiind].imshow(m)

        # # Show all ticks and label them with the respective list entries
        ax[taskind][hemiind].set_yticks(np.arange(len(alpha_values)), labels=alpha_values)
        ax[taskind][hemiind].set_xticks(np.arange(len(l1_ratio_values)), labels=l1_ratio_values)
        plt.xlabel('l1 ratio')
        plt.ylabel('alpha')
