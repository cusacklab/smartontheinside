...
for subjind=1:length(subjlist)
    ...
    %% Back transform ROIs
    fprintf('applywarp ');
    roilist={'rhcp_regions_v4_seed.nii','rhcp_regions_v4_target.nii'};
    ...
    fprintf('split masks ');
    [Vaal Yaal]=aas_spm_vol(fullfile(tbsspth,sprintf('%d_rhcp_regions_v4_target.nii.gz',subj)));
    
    targetmaskfn=fullfile(tbsspth,sprintf('%d_laura_target_masks.txt',subj));
    ...
    for ind=1:length(labels)
        Vaal.fname=fullfile(tbsspth,sprintf('%d_hcp_v4_%d.nii',subj,labels(ind)));
    ...
    cmd=['probtrackx2 --onewaycondition -P 5000 --forcedir --opd --os2t ' ...
        ...
        sprintf(' -x %s_rhcp_regions_v4_seed.nii.gz',tbssroot) ...
    ...
    parfor labind=1:length(labels)
        cmd=[sprintf('applywarp -i %s',fullfile(probtrackx2pth,sprintf('seeds_to_%d_hcp_v4_%d',subj,labels(labind)))) ...
            sprintf(' -w %s/%d_dti_FA_FA_to_target_warp ',tbsspth,subj) ...
            ' -r /usr/share/fsl/data/standard/FMRIB58_FA_1mm ' ...
            sprintf(' -o %s ',fullfile(probtrackx2pth,sprintf('MNI_seeds_to_%d_hcp_v4_%d',subj,labels(labind)))) ...
            ' --interp=nn'];
        system(cmd);
    end;
    ...
    cmd=sprintf('find_the_biggest %s/MNI_seeds_to_%d_hcp_* %s/MNI_hcp_v4_%d_biggest',probtrackx2pth,subj,probtrackx2pth,subj);
    system(cmd);
