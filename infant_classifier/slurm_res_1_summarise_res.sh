        ...
        if [[ " ${preterm_list[*]} " =~ " ${SUBJ} " ]]; then
            ...
            for SESSDIR in ${SUBJDIR}/ses-*/; do
                if [[ -d "$SESSDIR" ]]; then
                    ...
                    echo "Summarising results for classifier for $SUBJ "
                    
                    # Check for bedpostX outputs
                    if [[ -f "${DWIPTH}/${SUBJ}/${SESS}/dwi.bedpostX/merged_f1samples.nii.gz" ]]; then
                        
                        export SUBJ=$SUBJ 
                        sbatch --export=ALL, /home/chiaracaldinelli/smartontheinside/smartontheinside/infant_classifier/slurm_res_2_summarise_res.sh $SUBJ
