...
print(frontalregs_left)
print(frontalregs_right)
...
for hemiind, hemi in enumerate(['R','L']):
    ...
    roi_res[hemiind] = np.zeros((nsub, 334, 26)) # Prepare the output file for the avg for each frontal ROI (L and R, nsubj * 334 targets * 26 frontal ROIs)
    for sub in range(nsub):
        for target_roi in range(1, 360): # Load ROI file and calculate average
            ...
            if not target_roi in frontalregs_right:
                if not target_roi in frontalregs_left:
                    ...
                    if vox_res[hemiind] is None:
                        ...
                        all_seed_values =  vox_res[hemiind][sub, target_roi, :]
        print(vox_res)
