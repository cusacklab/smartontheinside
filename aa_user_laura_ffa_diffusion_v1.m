...
% Set number of splits - here ROIs - for aamod_diffusion_probtrackx
numrois=360;

% Only 5 seed ROIs for speed

aap.options.probtrackx.seedroirange=[1     4     5     6     7    18    21    22   103   106   107   118   119   120   121   122   125   126   127   128   129   130   132   133   134   135   136   137   138   153 ...
   154   155     157   160   163   167   172   173   175   176   177   178];
% remove 156 as missed in some subjects
aap.options.probtrackx.nsplits=length(aap.options.probtrackx.seedroirange);


aap.tasksettings.aamod_diffusion_probtrackx.makesplitsacrossrois=true;

% Hard links don't work on ssc-rt-scanner
aap.directory_conventions.linktype='hard';

%aap.tasksettings.aamod_unnormalise_targets.roi_path='/imaging/rcusack/laura_ffa_diffusion/rois/targets';
%aap.tasksettings.aamod_unnormalise_targets.rois=struct([]);

%aap.tasksettings.aamod_unnormalise_targets.rois(1).roi=struct('filename','hcp_regions_%d.nii','low',1,'high',numrois);
%aap.tasksettings.aamod_unnormalise_seeds.rois(1).roi=struct('filename','hcp_regions_%d.nii','low',1,'high',numrois);
