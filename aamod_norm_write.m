...
switch task
    ...
    case 'doit'
        ...
        %added to account for multiple sessions
        if nargin == 4,
            sess = varargin{2};
        end
        ...
        if aas_stream_has_contents(aap,'normalisation_seg_sn')
            ...
            trans = aas_getfiles_bystream(aap,aap.tasklist.currenttask.domain,cell2mat(varargin),'normalisation_seg_sn');
        elseif aas_stream_has_contents(aap,'forward_deformation_field')
            flags = aap.spm.defaults.normalise.write;
            trans = aas_getfiles_bystream(aap,aap.tasklist.currenttask.domain,cell2mat(varargin),'forward_deformation_field');
        else
        ...
        for streamind=1:length(streams)
            ...
            % describe outputs with diagnostic if reslicing regular
            % structural
            aap=aas_desc_outputs(aap,aap.tasklist.currenttask.domain,cell2mat(varargin),streams{streamind},wimgs);
            if strcmp(aap.options.wheretoprocess,'localsingle') && any(ismember(streams, 'structural'))
