                    ...
                    switch streamfiles(depind).streamlocation
                        ...
                        case 'local'
                            ...
                            for ind=1:length(streamfiles(depind).fns_dest_full)
                                ...
                                cmd_rsync=['cd ' streamfiles(depind).src '; rsync -t ' streamfiles(depind).fns{ind} ' ' streamfiles(depind).fns_dest_full{ind}];
                                if (streamfiles(depind).ismodified || strcmp(aap.directory_conventions.linktype,'none'))
                                    cmd=cmd_rsync;
                                else
                                    % This
                                    ...
                                    switch (aap.directory_conventions.linktype)
                                        case 'hard'
                                ...
                                [s w]=aas_shell(cmd);
                                if s || ~isempty(strfind(w,'failed to create hard link'))
                                    % Symlinking failed, so fall back to
                                    % rsync
                                    aas_log(aap,false,sprintf('Sym linking failed with %s\nso falling back to rsync copy',cmd));
                                    [s w]=aas_shell(cmd_rsync);
